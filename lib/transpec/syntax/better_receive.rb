  ...
  class Syntax
    class BetterReceive < Syntax
      ...
      def dynamic_analysis_target?
        super &&
          receiver_node &&
          [:better_receive, :better_not_receive].include?(method_name)
      end
      ...
      # subject.better_receive(:method_name).once.with do |block_arg|
      # end
      #
      # (block
      #   (send
      #     (send
      #       (send
      #         (send nil :subject) :better_receive
      ...
      # subject.better_receive(:method_name) do |block_arg|
      # end.once
      #
      # (send
      #   (block
      ...
      class ExpectBaseRecordBuilder < RecordBuilder
        param_names :better_receive, :negative_form_of_to
        ...
        def old_syntax
          syntax = if better_receive.any_instance?
                     'Klass.any_instance.'
                   else
                     'obj.'
                   end
          syntax << "#{better_receive.method_name}(:message)"
        end
        ...
        def new_syntax
          ...
          syntax << (better_receive.positive? ? 'to' : negative_form_of_to)
      ...
      class AllowRecordBuilder < ExpectBaseRecordBuilder
        ...
        def old_syntax
          syntax = super
          syntax << '.any_number_of_times' if better_receive.any_number_of_times?
          syntax << '.at_least(0)' if better_receive.at_least_zero?
          syntax
        end
      end

      class StubRecordBuilder < RecordBuilder
        param_names :better_receive

        def old_syntax
          syntax = "obj.#{better_receive.method_name}(:message)"
          syntax << '.any_number_of_times' if better_receive.any_number_of_times?
          syntax << '.at_least(0)' if better_receive.at_least_zero?
          syntax
        end
